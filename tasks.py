...
def check(ctx):
    ...
    with ctx.cd(os.path.dirname(__file__)):
        ...
        ctx.run("mypy --ignore-missing-imports api tests")
...
def test(ctx, no_coverage=False, html_report=False, no_colors=False):
    ...
    with ctx.cd(os.path.dirname(__file__)):
        if no_coverage:
            ctx.run("pytest --doctest-modules api tests", pty=not no_colors)
        else:
            ctx.run("coverage run --source=api -m pytest --quiet --doctest-modules api tests", pty=not no_colors)
